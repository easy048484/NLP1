...
def test_llm_fallback_resolves_name_buried_in_sentence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """ "서울 강남구 거주 김영수" 처럼 라벨 없이 문장 속에 묻힌 이름 케이스."""
    monkeypatch.setattr(
        requirement_checker, "extract_testator_name", lambda masked_text: "김영수"
    )
...
def test_llm_fallback_failure_keeps_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        requirement_checker, "extract_testator_name", lambda masked_text: None
    )
...
def test_check_requirements_end_to_end_with_llm_fallback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_check_requirements_regex_hit_reports_regex_method(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def _fail_if_called(masked_text: str):
        raise AssertionError("정규식이 이미 찾았으면 LLM을 호출하면 안 된다")
