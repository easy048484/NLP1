...
⚠️ **데모 비핵심(2026-08-30, `spec.py` 참고)** — 서비스의 데모 초점은 "상속"
이라 이 에이전트는 전면에 안 나온다. 엔진 자체는 검증돼 있고 라우팅되면
정상 동작하지만, `keywords`에서 "연금"을 뺀 여파로 "연금 계산해줘" 같은
순수 연금 질의는 이제 라우팅 자체가 안 되는 상태다(아래 "빌드 히스토리"
참고) — 지금은 의도된 트레이드오프로 보이나 재논의 여지가 있다.
...
- ⚠️ **"연금" 키워드 제거로 순수 연금 질의가 라우팅 안 됨(알려진 트레이드오프)**
  ...
  연금 질의는 키워드 후보 0개가 되어 이 에이전트가 아예 실행되지 않는다
  (`tests/test_retirement_planner_keyword_collision.py` 참고). 위 "데모
  비핵심" 스코프 결정상 당장은 허용됐지만, 나중에 이 에이전트를 다시
  전면에 내세우게 되면 재논의 필요.
