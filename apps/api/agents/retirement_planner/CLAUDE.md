...
⚠️ **데모 범위에서 제외됨(2026-08-30, 팀 계획서 결정, `spec.py` 참고)** —
서비스의 데모 초점은 "상속"이다. `keywords=[]`로 비워서 사용자가 어떤
문구로도 이 에이전트에 직접 도달할 수 없게 막았다. 엔진·대화 로직은 전부
그대로 보존돼 있다(engine.py 등 삭제 안 함 — 로드맵/발표자료용).
`is_stub=True`도 되돌렸지만, **`is_stub` 하나만으로는 라우팅이 안 막힌다는
걸 실측으로 확인했다** — `orchestrator.planner.classify()`의 Standard 경로
(키워드 후보 1개)는 `is_stub`을 아예 확인하지 않는다. 실질적인 차단 장치는
`keywords=[]`다.

⚠️ **다만 `asset_organizer` → `retirement_planner` 핸드오프(Fast Path)는
그대로 동작한다** — `pending_handoff`는 키워드 매칭보다 먼저 확인되고
`registry.get_optional()`로만 대상 존재 여부를 보기 때문에 `keywords=[]`의
영향을 안 받는다. "사용자가 먼저 말 걸어서 도달"만 막혔지, "자산 정리
체크리스트 완료 후 자동으로 이어짐"은 여전히 살아있다 — 이게 "데모 제외"
의도와 맞는지는 판단이 필요해 보여 그대로 남겨뒀다(팀 결정 대기).

⚠️ **이 변경으로 `tests/test_orchestrator_planner.py`의 테스트 7개가
깨진다(미수정, 이 에이전트 소유가 아닌 파일이라 손대지 않음)** — 그 파일이
"은퇴 준비하면서 상속세도 궁금해요" 같은 문구로 retirement_planner를
tax_calculator와 함께 "키워드 후보 2개 이상 → Full Pipeline" 시나리오를
구성하는 데 써왔다(병렬 실행, 에이전트 실패 격리, financial_profile 병합,
검증 결과 합성 등 retirement_planner 자체와 무관한 오케스트레이터 메커니즘
테스트용 픽스처로 사용). `keywords=[]`가 되면서 이 문구가 더 이상 키워드
후보 2개를 못 만들어 그 테스트들의 전제 자체가 깨진다. 조용히 억지로
맞추지 않고 그대로 보고함 — 픽스처로 쓸 다른 에이전트 쌍을 고르거나 다른
방식으로 테스트를 재구성하는 건 그 파일 담당자(지원) 결정.
...
- **`tests/test_orchestrator_planner.py`의 테스트 7개가 이번 데모 제외
  변경으로 깨져 있다(미수정)** — 위 상단 경고 참고. 이 에이전트 소유가
  아닌 파일이라 손대지 않고 그대로 보고만 해뒀다.
...
- **실제 대화 흐름 연결을 오케스트레이터로 실행해서 검증** — 단위 테스트로
  ...
  빌드 히스토리 참고).
- **라우팅 키워드 "연금" 제거(팀원이 develop에 직접 커밋, PR #45 병합 직후)**
  — 위 핸드오프 조사 중 발견된 "퇴직연금"/"연금으로 받을게요" 등과의
  substring 충돌 때문에, 팀원이 커밋 45253e9에서 `spec.py`의 키워드를
  `["은퇴", "노후", "연금"]` → `["은퇴", "노후"]`로 좁혔다 — 대체 키워드
  없이 "연금"을 통째로 뺀 것. 같은 커밋에서 "데모 비핵심(서비스는 '상속')"
  이라는 스코프 결정도 함께 남겼다. **근본 원인(오케스트레이터
  `registry.match_keywords()`의 substring 매칭 방식 자체)은 여전히 손대지
  않았다.**
  - ⚠️ 이 해결 방식을 실제 라우팅 테스트로 검증하는 과정에서, "연금
    계산해줘"/"제 예상 연금이 얼마나 될까요" 같은 순수 연금 질의가
...
- **금액 콤마 파싱 버그 수정(P0-3)** — "3,200만원"을 콤마 뒤 "200만원"으로만
  읽어 앞자리가 통째로 사라지던 버그. `_parse_amount()`에 콤마를 하나의
  숫자로 합쳐 읽는 정리 단계를 추가(asset_organizer/extractor.py와 동일한
  로직의 로컬 복제본이라 양쪽 다 고침, `agent.py` 상단 주석 참고).
- **팀 계획서 결정(데모 범위 제외) 반영** — `spec.py`의 `keywords`를 완전히
  비우고 `is_stub=True`로 되돌렸다. 위 상단 경고 참고 — `is_stub`만으로는
  라우팅이 안 막힌다는 것과, 핸드오프(Fast Path)는 여전히 동작한다는 것,
  `test_orchestrator_planner.py` 테스트 7개가 이 변경으로 깨졌다는 것을
  전부 실측으로 확인·보고했다.
