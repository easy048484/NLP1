...
- **이미지 판독 "unclear" 필드의 구조적 위험은 남아 있다** — 지금은 소비하는
  ...
  정형화하거나 마스킹할 것.
- ⚠️ **부채 후속질문 게이트가 퇴직연금 후속질문을 가로챌 수 있다(실측 발견,
  아직 미수정)**: `_run_turn()`의 `was_awaiting_liability_followup` 판단이
  "한 번 물어봤는지"가 아니라 `_liabilities_needing_followup(state)`(부채
  필드가 여전히 비어 있는지)로 매 턴 다시 계산된다 — 부채가 단순 모드로
  영구히 비어 있으면(예: "몰라요"로 답해 monthly_payment/end_age를 끝내
  못 채운 경우) 이 조건이 **그 이후 모든 턴에서 계속 True**로 남는다.
  liability 체크가 pension 체크보다 먼저라, 부채+퇴직연금이 같은 대화에
  같이 있으면 퇴직연금 후속질문에 대한 답("연금으로 65살부터...")이
  liability 쪽 로직에 먼저 먹혀버려 소득 전환이 조용히 무시된다(단위
  테스트는 한 번에 하나의 흐름만 손으로 세팅해서 이 상호작용을 못
  잡았다 — 실제 오케스트레이터로 부채+퇴직연금을 함께 넣고 나서야
  발견). `pension_followup_resolved`처럼 부채 쪽에도 별도의 "이미 한 번
  물어봤음" 플래그를 둬서 "아직 안 채워짐"과 "이미 답변 받았지만 못
  채웠음"을 구분해야 한다.
...
- **핸드오프 부재 발견 및 수정**: 체크리스트가 끝나도 `next_action`/`handoffs`가
  ...
  - **근본 원인(오케스트레이터의 substring 키워드 매칭 방식 자체)은 팀 논의
    대상이라 손대지 않았다** — `registry.match_keywords()`는 여전히 단순
    `keyword in user_message` 매칭이고, 이런 충돌이 다른 에이전트 키워드
    조합에서도 또 생길 수 있는 구조적 문제다. **다만 이번엔 "연금" 하나가
    일으키는 이 특정 충돌만, 우리 담당 에이전트인 retirement_planner의
    키워드 목록 조정으로 임시 방어했다**(`retirement_planner/spec.py`:
    단독 "연금" → "연금 계산"/"예상 연금"). 완전한 해결책이 아니다 — 상세는
