...
- **"빚" 포괄 상담 의도 오탐 수정** — 실측 재현된 버그: 사후 모드 첫 턴에서
  "어머니가 돌아가셔서 재산이랑 빚을 한번 정리해두려고 해요."라고만 말해도
  `_LIABILITY_KEYWORDS`의 "대출" 유형 키워드 중 하나인 "빚"이 곧바로 잡혀
  대출 존재가 확정되고 대출 금액을 되물었다 — 구체적 보유 항목이 없는
  포괄적 상담 의도일 뿐인데도(자산 쪽은 "재산"/"자산"이 키워드 자체가
  아니라 이 문제가 없었다). "대출"/"융자"/"카드론" 등과 달리 "빚"은
  일상어라 "부채"의 대용으로 포괄 의도 문장에도 흔히 쓰인다는 게 원인 —
  extractor.py에 `_match_liability_type`이 "빚" 키워드에 한해서만
  `_is_generic_liability_intent`로 한 번 더 거르도록 했다: "정리"/"확인" +
  의향형 어미(-려고/-고 싶/-줄래/-볼까 등, `_ORGANIZE_INTENT_RE`)가 있고
  "있다"/"남다" 같은 실제 존재 진술(`_EXISTENCE_VERB_RE`)이 없을 때만
  이 키워드 매칭을 무시한다 — "빚이 4천만원 있어요"/"빚이 좀 있는데
  정리하고 싶어요"는 존재 진술이 있어 그대로 확정되고, "대출이 있어요"/
  "카드대출이 남아 있어요"/"대출은 없어요"는 "빚"이 아닌 다른 키워드로
  매칭돼 이 가드 자체를 타지 않는다. orchestrator 라우팅 키워드(spec.py,
...
- **최종 확정 요약에 보험 합계 제외 이유 명시** — review 화면
  ...
  표시 문구 추가.
- **P0: 부채가 "기타" 자산으로 중복 등록되던 버그 수정** — 실측 재현:
  "안심상속 조회 결과 예금 8천만 원, 아파트 5억 원, 카드대출 2천만 원이
  확인됐어요. 자산과 부채를 정리해주세요." → 카드대출 2천만원이
  `extract_liabilities()`로 정확히 "대출"로 잡히는 동시에, 같은 텍스트를
  독립적으로 훑는 `extract_financial_slots()`은 "카드대출..." 세그먼트를
  자산 키워드 사전에 없어 unresolved로 분류하고, 그걸 그대로 asset LLM
  폴백에 넘겨 LLM이 "기타" 자산(2천만원)으로 반환할 수 있었다 — 같은
  금액이 자산에도 부채에도 잡혀 자산 합계가 부채 금액만큼 부풀려졌다
  (600m/20m/580m이어야 할 게 620m/20m/600m으로).
  - **수정**: `extract_financial_slots()`가 unresolved 세그먼트를 LLM에
    보내기 전에 `_match_liability_type(segment) is not None`인(대출·
    카드론·전세자금대출·임대보증금반환채무로 식별 가능한) 세그먼트를
    제외한다 — 그 세그먼트의 소유권은 오직 `extract_liabilities()`에게만
    있다(같은 원문을 독립적으로 재처리하므로 정보 유실 아님). 남은
    unresolved가 전부 부채 세그먼트였다면 LLM 자체를 부르지 않고
    `status`를 다시 계산한다.
  - **금액만 같다는 이유의 dedupe는 명시적으로 금지**했다(요구사항) —
    "예금 2천만원, 대출 2천만원"처럼 우연히 금액이 같은 정상 케이스는
    둘 다 보존돼야 하므로, 값 비교가 아니라 세그먼트 자체의 부채
    식별 가능성만으로 걸러낸다.
  - `_build_system_prompt()`에도 "대출·카드론 등 부채는 assets에 넣지
    마라" 방어 지시를 보조로 추가했다 — 단 이것만으로는 고쳐지지 않는다
    (실제로 문제가 되는 세그먼트는 애초에 LLM에 보내지지 않는 구조적
    제외가 진짜 수정). 이미지 판독 경로(`extract_from_image`)는 별도
    프롬프트/파서를 쓰고 이번 재현 시나리오(텍스트 입력)와 무관해
    손대지 않았다 — 같은 클래스 문제가 있는지는 별도 확인 필요(미해결
    항목 후보).
