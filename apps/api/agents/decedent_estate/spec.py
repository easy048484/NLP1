"""decedent_estate 의 라우팅 선언 (orchestrator/registry.py 참고). agent.py 는 건드리지 않습니다."""

from orchestrator.registry import AgentSpec
from schemas import AgentAxis, AgentName

from .agent import run

SPEC = AgentSpec(
    name=AgentName.DECEDENT_ESTATE,
    axes=[AgentAxis.POST_DEATH],
    description="유언장의 종류·법적 요건·효력을 점검 (자산 목록 정리는 asset_organizer 담당)",
    # planner._classify_prompt() 는 example_utterances[:3] 만 LLM few-shot 으로
    # 쓴다 — 새 예시는 반드시 앞 3개 자리에 넣어야 효과가 있다. 기존 예시는
    # 뒤에 그대로 보존(총 7개, 기존 커버리지 유지).
    #
    # 2026-09-05 LLM-first 라우팅 전환(키워드 매칭이 후보를 좁히지 않고 LLM이
    # 전체 등록 에이전트를 보고 고름)으로 "유언"/"유언장" 등 키워드를 전혀
    # 쓰지 않은 발화도 이 에이전트가 스스로 고를 수 있어야 한다 — 맨 앞에
    # 그런 예시를 추가했다("문서"/"효력"만으로 유언장 점검임을 알아채야 함).
    #
    # 2026-09-07 자연어 QA에서 발견: planner._classify_prompt()는
    # example_utterances[:3]만 few-shot으로 쓰는데(위 주석·registry.py), 그
    # 앞 3자리가 전부 "효력이 있는지" 계열이라 "집을 다 뒤졌는데 유언장을
    # 못 찾겠어요"류(유언장 존재 자체가 불확실/부재)는 few-shot 커버리지가
    # 없어 LLM이 5회 연속 heir_navigator(상속 절차 안내)로 잘못 보냈다 —
    # "유언장 없이 돌아가셨는데 절차가 어떻게 되나요?" 예시가 이미 있었지만
    # 7번째 자리라 실제로는 전혀 쓰이지 않고 있었다. 두 번째 자리를 그
    # 계열로 바꿔 넣는다 — 나머지 순서/문항 수는 그대로 유지.
    example_utterances=[
        "아버지가 손으로 남긴 문서가 있는데 이게 효력이 있는지 모르겠어요",
        "집을 다 뒤졌는데도 유언장을 못 찾겠어요",
        "유언장을 직접 쓰려고 하는데 형식 요건을 알려주세요",
        "자필로 남긴 유언장이 효력이 있는지 확인하고 싶어요",
        "녹음으로 남긴 유언도 법적으로 효력이 있나요?",
        "유언장이 있는데 효력이 있는지 봐주세요",
        "자필 유언장 요건이 뭔가요?",
        "유언장 없이 돌아가셨는데 절차가 어떻게 되나요?",
    ],
    # "자산정리"는 asset_organizer 로 넘겼다 — 이 에이전트는 유언장 자체만 본다.
    keywords=["유언", "유언장", "유언 요건", "유언 효력", "자필증서", "공정증서"],
    requires=[],
    # 유언장 유무/유효성 판정(will_status) — tax_calculator·heir_share_analyzer 가
    # 같은 턴에 뽑히면 이 결과 뒤에 돌고, AgentInput.will_status 로 판정 요약을 받는다.
    produces=["will_status"],
    entrypoint=run,
)
