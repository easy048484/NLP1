...
본 시스템의 형식 엄격주의 전제는 대법원 2004다35533, 2005다57899 등이 확인한
법리("법정 요건·방식에 어긋난 유언은 진정한 의사에 합치해도 무효")에 따른다.
이미 "요건 하나라도 빠지면 무효"로 구현돼 있는 시스템 전체의 설계 근거라
개별 요건 판례처럼 rules/precedents.json에 등재하지는 않는다.
...
4. 마스킹(주민번호·계좌·전화 치환) 이전의 원본 텍스트를 LLM API에 보내지 않는다.
   ...
   ⚠️ **예외 — 유언장 사진 판독(image_reader.py, 2026-08-25)**: 이 원칙은
   텍스트를 전제로 한다. 이미지에는 적용할 방법이 구조적으로 없다 — 이미지를
   마스킹하려면 먼저 읽어야 하는데, 읽는 행위(Anthropic API 호출) 자체가
   이미 전송이기 때문이다. 그래서 사진 판독 단계만 이 원칙의 적용 대상에서
   제외하고, 아래 네 가지로 원칙의 정신(민감정보를 불필요하게 외부로 보내지
   않는다)을 대신 지킨다:
   - 판독 단계는 "마스킹 이전 단계"로 간주한다 — 어차피 사진 전체가 통째로
     전송된다는 사실 자체를 감춘다는 뜻이 아니라, 그 단계는 원칙 4의 검증
     범위 밖이라는 뜻이다.
   - **판독 결과 텍스트부터는 기존 mask_text()가 그대로 적용된다.** 사진에서
     뽑은 값(성명·주소·날짜)이 재구성 텍스트로 만들어져
     requirement_checker.py 파이프라인에 들어가는 순간부터는, 그 안에서
     LLM 폴백이 다시 호출될 때 여느 텍스트 입력과 동일하게 마스킹을 거친다.
   - **원본 이미지는 저장하지 않는다** — DecedentState 에 이미지 필드 자체가
     없다(state.py, 방침 B와 동일한 구조적 차단).
   - **사용자에게 전송 사실을 업로드 전 고지한다** — 프론트 UI 요구사항,
     docs/진행상황.md 의 사진 판독 절 참고. 주민등록번호·전화번호·계좌번호는
     가리고 올리도록 권고하지만 강제하지는 않는다 — 강제하지 않는 이유는
     이중 방어(마스킹은 판독 이후 텍스트에 대해서만 가능) 때문이다.
...
- **유언 방식 분기**(rules/will_types.json, will_types.py)가 agent.run() 맨 앞단에
  ...
  - **none("유언장이 없거나 찾지 못했습니다")** → 요건 판정 대상 자체가 없으므로
    ...
    - **상속인 범위·지분·유류분은 답하지 않는다** — heir_navigator 영역이라
      ...
      없는 경우 일반적으로 법정상속 절차를 따릅니다"까지만 말하고, 필요하면
      채팅으로 돌아가 다시 물어보라고 안내한다(아래 라우터 참고). (절대 원칙 2
      무단정 유지 — "유언장이 없으니 법정상속입니다" 금지)
    ...
    - **heir_navigator로의 직접 next_action 핸드오프는 없다** (2026-08-25 제거).
...
7) **유언장 사진 판독 1단계**(image_reader.py, 2026-08-25)가 추가됨:
   사용자가 사진(AgentInput.image_base64/image_media_type)을 올리면 성명·
   주소·작성연월일·날인(도장/지장) 4필드를 값+확신도(high/low/none)로
   추출해, `agent._resolve_photo_intake()`가 재구성 텍스트로 바꿔
   `_run_handwritten_pipeline`(review 모드만, prepare 모드는 대상 아님)에
   넘긴다. **기존 requirement_checker.py/masking.py/result_formatter.py는
   한 글자도 안 고쳤다** — 재구성 텍스트가 기존 텍스트 입력과 완전히 동일한
   경로를 타기 때문이다(방침 A).
  - **자서(전문 자서)는 판독 대상이 아니다.** 필적 감정 영역이라 LLM이
    판정하게 하지 않는다 — 애초에 handwriting 요건은 이 기능 이전부터
    `extraction_type: "user_confirm"`이라 항상 사용자에게 직접 물었으므로,
    이 기능이 그 경로를 건드릴 필요 자체가 없었다.
  - **확신도 3단**: high(질문 없이 즉시 채움) / low(사진에서 이렇게
    읽었는데 맞는지 확인 질문) / none(값 자체가 없어 조용히 absent — 물어도
    소용없다). low 확인은 `rules/requirements.json`의 `photo_confirm_templates`
    (기존 `requirements[].followup`과는 별개의 새 최상위 키 — 정적 선택지만
    지원하는 기존 구조를 안 건드리기 위해 분리했다)로 만든 동적 질문이다.
  - **오디오(녹음 유언)는 범위 밖이다** — Anthropic Messages API가 오디오
    입력을 지원하지 않는다(공식 문서 기준 이미지 JPEG/PNG/GIF/WebP·PDF만
    지원, SDK 저장소의 오디오 지원 요청 이슈도 "Closed as not planned",
    2026-08-25 확인). 녹음은 기존 대본 텍스트 입력 그대로 유지한다.
  - 원본 이미지는 DecedentState에 담기지 않는다 — `photo_draft`에는 4필드의
    짧은 문자열/열거값만 왕복한다(위 절대 원칙 4의 예외 조항 참고).
