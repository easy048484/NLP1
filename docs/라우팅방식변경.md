...
# 2차 변경 — LLM-first 라우팅 (2026-09-06, 브랜치 feat/llm-routing-context)

PR #100(feature/llm-first-routing)과 PR #130(feat/llm-first-routing)이 같은 날 같은 목표로 따로 만들어졌고 #130이 먼저 머지됐다. 이 브랜치는 #130이 들어간 develop 위에 **#100의 설계를 다시 얹은 것**이다 — 카탈로그(spec.py → 프롬프트)는 두 구현이 같았고, 차이는 "LLM에게 대화 상태를 얼마나 보여주는가"였다.
...
`spec.py` 마다 "실측 재현"이라며 키워드를 늘려온 것(`은행/예금/계좌…`, `자녀에게만/분쟁/형평…`)은 관성을 막으려다 하이재킹을 키우는 땜질이었다. #130은 후보 개방으로 관성을 풀었지만, LLM에 직전 에이전트 **이름**만 줘서 하이재킹은 "답변 대기 신호를 내는 에이전트"(decedent_estate)만 코드 규칙으로 보호됐다.

## 무엇이 바뀌었나 (`orchestrator/planner.py`)

```
classify(user_message, pending_handoff, pending_reply_agent, last_agent, default_agent, axis, history)
  (0) pending_reply_agent 있음 → 그 에이전트 1개 (LLM 미호출, 하드 규칙)
...
1. **후보 전체 개방** — LLM 의 `enum` 이 키워드 후보가 아니라 등록된 에이전트 전부(`is_stub` 제외). 관성 해소. (#130과 동일)
2. **대화 상태를 라우터에 배선** — `router.node_classify` 가 `session.history` 를 넘기고, 마지막 assistant 발화(=직전 질문, 끝 500자)·직전 에이전트·핸드오프 예정·키워드 힌트·상담 구분이 user 프롬프트에 들어간다. 하이재킹 해소.
...
5. **`pending_reply_agent` 는 하드 규칙 유지** — #110에서 도입한 "답변 대기 중" 계약. 에이전트가 명시적으로 자료를 요청한 상태를 LLM 판단이 뒤집으면 안 되므로 LLM 앞단에 둔다. 소프트 신호(핸드오프)가 하드 신호(답변 대기)를 이길 수 없으므로 **둘이 동시에 있으면 답변 대기가 이긴다** — #110 당시 "핸드오프 우선"이던 순서가 여기서 뒤집혔다 (`tests/test_orchestrator_planner.py::test_pending_reply_agent_wins_over_pending_handoff`).
6. **`is_stub` 이 라우팅을 막는다** — LLM 후보에서 제외. `retirement_planner` 의 `keywords=[]` 차단은 규칙 경로용으로 그대로 유효.

프롬프트 구조: 에이전트 카탈로그 + 판단 규칙은 **system**(턴마다 동일 → 캐시 가능), 대화 상태 + 사용자 메시지는 **user**. 모델은 `CLAUDE_ROUTER_MODEL` 로 따로 지정 가능(비우면 `CLAUDE_MODEL`). `planner._classify_prompt(candidates)` 는 각 팀 spec 테스트용 하위호환 별칭으로 남겨뒀다(시스템 프롬프트를 돌려줌).

## 각 에이전트 담당자가 할 일

라우팅 판단 재료가 키워드에서 **`spec.py` 의 `axes` / `description` / `example_utterances`** 로 옮겨갔다. 이 세 값이 그대로 LLM 프롬프트다. 자기 `spec.py` 에서:

- `description` 에 "무엇을 하지 않는지"(인접 에이전트로 밀어낼 주제)를 함께 적을 것 — asset_organizer, heir_navigator, tax_calculator, heir_share_analyzer 는 반영됨
- `example_utterances` 앞 3개만 프롬프트에 들어감(`planner._catalog_lines`). 법률 용어 없는 평범한 말투로 서로 다른 유형 3개
- `axes` 가 실제 담당 범위와 맞는지 — 생전·사후 겸용이면 `[PRE_NEED, POST_DEATH]` 둘 다 (#109)
- `keywords` 는 이제 LLM 없을 때의 폴백 + 프롬프트 힌트다. 오탐 유발 단어(`재산/절차/정리/은행/계좌…`)는 정리 예정
- 사용자 답을 기다리는 질문을 던졌으면 `next_action="await_user_confirmation"` 을 내는 것이 가장 확실하다(하드 규칙). 안 내더라도 LLM 이 직전 발화를 보고 이어가지만, 그건 확률적 판단이다.

## 비용

턴마다 라우팅 LLM 호출 1회가 추가된다(`effort="low"`, `max_tokens=1024`, 출력은 이름 몇 개). 에이전트 자체가 이미 턴마다 LLM 을 부르므로 체감 지연은 크지 않지만, 필요하면 `CLAUDE_ROUTER_MODEL` 로 라우팅만 가벼운 모델로 내릴 수 있다. pending_reply_agent 턴은 LLM 을 부르지 않는다.

## 테스트

- `tests/test_orchestrator_llm_routing.py` — `claude.extract` 를 가짜로 바꿔 LLM 경로 검증 (하이재킹·관성·핸드오프 힌트·답변 대기 선점·복수 선택·폴백·required 모드·system 프롬프트 불변·route() 2턴 end-to-end)
- `tests/test_orchestrator_planner.py` — classify() 배관(키워드 0/1개여도 LLM 호출, 후보 전체, 규칙 경로 폴백)과 키워드 규칙 경로(LLM 없음)
